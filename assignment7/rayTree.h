...
class CellFace {
   public:
    // CONSTRUCTOR & DESTRUCTOR
    CellFace() {}
    CellFace(Vec3f _a, Vec3f _b, Vec3f _c, Vec3f _d, Vec3f _normal,
             Material *m) {
        a = _a;
        b = _b;
        c = _c;
...
class RayTree {
   public:
    ...
    static void AddTransmittedSegment(const Ray &ray, float tstart,
                                      float tstop) {
        if (!activated) return;
    ...
    static void AddHitCellFace(Vec3f a, Vec3f b, Vec3f c, Vec3f d, Vec3f normal,
                               Material *m) {
        if (!activated) return;
        hit_cells.addCellFace(CellFace(a, b, c, d, normal, m));
    }
    static void AddEnteredFace(Vec3f a, Vec3f b, Vec3f c, Vec3f d, Vec3f normal,
                               Material *m) {
        if (!activated) return;
   ...
   private:
    // HELPER FUNCTIONS
    static void paintHelper(const Vec4f &m, const Vec4f &s, const Vec4f &r,
                            const Vec4f &t);
    static void Clear() {
        main_segment.Clear();
