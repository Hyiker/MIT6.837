...
class Particle {
   public:
    ...
    Particle(Vec3f p, Vec3f v, Vec3f _color, Vec3f _dead_color, float m,
             float lifespan) {
        position = p;
        last_position = p;
        velocity = v;
   ...
   private:
    ...
    Vec3f velocity;
    Vec3f color;
    Vec3f dead_color;  // color interpolates from color to dead_color as clock
                       // ticks down
    float mass;
    float timeToDeath;
};
