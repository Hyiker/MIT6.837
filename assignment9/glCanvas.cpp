...
void GLCanvas::initialize(Parser* _parser, float _refresh, float _dt,
                          int _integrator_color, int _draw_vectors,
                          float _acceleration_scale, int _motion_blur) {
    parser = _parser;
    refresh = _refresh;
    dt = _dt;
...
void GLCanvas::display(void) {
    ...
    // Draw the systems
    for (int i = 0; i < parser->getNumSystems(); i++) {
        parser->getSystem(i)->Paint(dt, integrator_color, draw_vectors,
                                    acceleration_scale, motion_blur);
    }
