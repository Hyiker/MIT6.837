...
void System::Paint(float dt, int integrator_color, int draw_vectors,
    ...
    int num_particles = particles->getNumParticles();
    for (int i = 0; i < num_particles; i++) {
        particles->Get(i)->Paint(dt, integrator_color, draw_vectors,
                                 motion_blur);
    }
    ...
    if (draw_vectors) {
        ...
        for (int i = 0; i < num_particles; i++) {
            ...
            Vec3f b =
                forcefield->getAcceleration(a, p->getMass(), current_time);
            // scale the acceleration (it's often too large to visualize
            // directly)
